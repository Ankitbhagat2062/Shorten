...
import { getColor, transformToSingleUrlData } from "./analyticsUtils";
...
export const StatsCard = ({ icon: Icon, title, value, change, changeType, delay, suffix = "" }) => {
  ...
  console.log("Value received in StatsCard:", value);
...
export const UrlTableRow = ({ url, onEdit, onDelete, onCopy, delay }) =>{
  const { urlData, setSingleUrlData } = useUrlStore();
  const onclickNavigate = (shortName) => {
    const singleUrldata = transformToSingleUrlData(urlData, shortName);
    setSingleUrlData(singleUrldata);
    window.open(`/analytics/${url.shortName}`, '_blank');
  };
  return  (
    <tr onClick={() => onclickNavigate(url.shortName)}
      className="border-b border-white/10 hover:bg-white/5 transition-colors duration-200 animate-fade-in-up"
      style={{ animationDelay: `${delay}ms` }}
    >
      <td className="py-4 px-1 cursor-pointer">
...
export const ProgressBarChart = ({ data, delay = 0 }) => {
  ...
  const maxValue = Math.max(...safeData.map((d) => d?.value ?? 0), 0);
...
export const LineChart = ({ data, dataKey, color, height = 200, delay = 0, xKey = 'hour' }) => {
  const safeData = Array.isArray(data) ? data : [];
  const maxValue = Math.max(...safeData.map((d) => d?.[dataKey] ?? 0), 0);
  const points = safeData.map((d, i) => ({
    x: (i / (safeData.length - 1 || 1)) * 100,
  ...
  if (!safeData.length || maxValue <= 0) {
    return (
      <div className="animate-fade-in-up" style={{ animationDelay: `${delay}ms` }}>
        <div className="text-white/40 text-center text-sm">No data available</div>
...
export const GoogleMapView = ({ locations }) => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      // load maplibre-gl only on client
      ...
      const maxClicks = Math.max(
        ...((locations || []).map((loc) => loc?.clicks || 0)),
        1
      );

      const points = (locations || []).filter(
        (l) => typeof l?.lng === 'number' && typeof l?.lat === 'number'
      );

      map.on('load', () => {
        points.forEach((p) => {
          const intensity = (p.clicks || 0) / maxClicks;
          ...
          el.style.background = `rgba(102, 126, 234, ${0.15 + intensity * 0.35})`;
          el.style.border = `2px solid rgba(118, 75, 162, ${0.35 + intensity * 0.4})`;
          el.style.transform = 'translate(-50%, -50%)';
          el.title = `${p.country || p.countryCode}: ${(p.clicks || 0).toLocaleString()} clicks`;
  ...
  return (
    <div
      ref={(el) => {
        // match MapLibre expected ref object
...
export const AnimatedCounter = ({ value, duration = 1500 }) => {
  ...
  useEffect(() => {
    const raw = value ?? 0;
    const end = Math.trunc(Number(String(raw).replace(/,/g, '')));

    // If value is 0 (or NaN), don't run the animation; render 0 immediately.
